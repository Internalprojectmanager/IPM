@extends('layout.app') 
@section('title') {{$project->name}} - {{$release->name}} {{number_format($release->version, 1)}}
| {{env('APP_NAME')}}
@endsection
...
@section('content')
    ...
    <div class="row">
        ...
        <form action="{{route('updaterelease', [ $project->path, $release->path, $release->version])}}" method="post">
            {{ csrf_field() }}
            <div class="row">
            ...
            <div class="form-group col-md-6">
                ...
                <textarea rows="4" cols="50" name="description" class="form-control" id="release_description">{{$release->description}}</textarea>

            </div>
            <div class="form-group col-md-6">
                <label class="edit-title" for="project_code">Version</label>
                <input type="text" class="form-control" name="version" id="release_version" value="{{$release->version}}">
            </div>
            <div class="form-group col-md-6">
                ...
                <input type="text" required class="form-control datepicker" autocomplete="off" id="deadline" name="deadline" placeholder="YYYY/MM/DD"
                    value="{{date('Y-m-d', strtotime($release->deadline))}}">
            ...
            <div class="form-group col-md-12">
                ...
                <input type="text" class="form-control input-text-modal" name="specification" id="" value="{{$release->specificationtype}}">
            </div>
            <div class="form-group col-md-6" align="left">
                <button type="button" class="btn-cancel" onclick="window.location.assign('{{route('overviewproject')}}')">Close
                        </button>
            </div>
